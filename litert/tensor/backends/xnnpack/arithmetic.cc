...
absl::Status OpMixin<AddOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const AddOperation& op_data, op.As<AddOperation>());
...
absl::Status OpMixin<MulOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(auto io, PrepareBinaryIO(op, ctx, op_name));
  LRT_TENSOR_ASSIGN_OR_RETURN(const MulOperation& op_data, op.As<MulOperation>());
  FusedActivation activation = op_data.activation;
  LRT_TENSOR_ASSIGN_OR_RETURN(auto params,
                              BuildBinaryParams(activation, op_name));
...
absl::Status OpMixin<SubOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(auto io, PrepareBinaryIO(op, ctx, op_name));
  LRT_TENSOR_ASSIGN_OR_RETURN(const SubOperation& op_data, op.As<SubOperation>());
  FusedActivation activation = op_data.activation;
  LRT_TENSOR_ASSIGN_OR_RETURN(auto params,
                              BuildBinaryParams(activation, op_name));
...
absl::Status OpMixin<DivOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(auto io, PrepareBinaryIO(op, ctx, op_name));
  LRT_TENSOR_ASSIGN_OR_RETURN(const DivOperation& op_data, op.As<DivOperation>());
  FusedActivation activation = op_data.activation;
  LRT_TENSOR_ASSIGN_OR_RETURN(auto params,
                              BuildBinaryParams(activation, op_name));
...
absl::Status OpMixin<LeakyReluOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(auto io, PrepareUnaryIO(op, ctx, op_name));
  LRT_TENSOR_ASSIGN_OR_RETURN(const LeakyReluOperation& data, op.As<LeakyReluOperation>());
  xnn_unary_params params;
...
absl::Status OpMixin<GeluOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(auto io, PrepareUnaryIO(op, ctx, op_name));
  LRT_TENSOR_ASSIGN_OR_RETURN(const GeluOperation& op_data, op.As<GeluOperation>());
...
absl::Status OpMixin<SoftmaxOperation, XnnpackMixinTag>::ToXnnpack(
    const graph::Operation& op, XnnpackBuildContext& ctx) const {
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(const SoftmaxOperation& op_data, op.As<SoftmaxOperation>());
...
absl::Status OpMixin<AveragePool2DOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const AveragePool2DOperation& op_data, op.As<AveragePool2DOperation>());
  if (op.inputs.size() != 1) {
    return absl::InvalidArgumentError(
...
absl::Status OpMixin<MaxPool2DOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const MaxPool2DOperation& op_data, op.As<MaxPool2DOperation>());
  if (op.inputs.size() != 1) {
    return absl::InvalidArgumentError(
        absl::StrFormat("%s expects 1 input", op_name));
  }
...
absl::Status OpMixin<Conv2DOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const Conv2DOperation& op_data, op.As<Conv2DOperation>());
...
absl::Status OpMixin<DepthwiseConv2DOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const DepthwiseConv2DOperation& op_data, op.As<DepthwiseConv2DOperation>());
  if (op.inputs.size() < 2 || op.inputs.size() > 3) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "%s expects 2 or 3 op.inputs (input, filter[, bias])", op_name));
  }
...
absl::Status OpMixin<FullyConnectedOperation, XnnpackMixinTag>::ToXnnpack(
    const graph::Operation& op, XnnpackBuildContext& ctx) const {
  const absl::string_view op_name = op.GetName();
  LRT_TENSOR_ASSIGN_OR_RETURN(const FullyConnectedOperation& op_data, op.As<FullyConnectedOperation>());
  if (op.inputs.size() < 2 || op.inputs.size() > 3) {
    return absl::InvalidArgumentError(absl::StrFormat(
...
absl::Status OpMixin<BatchMatMulOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const BatchMatMulOperation& op_data, op.As<BatchMatMulOperation>());
...
absl::Status OpMixin<MeanOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const MeanOperation& op_data, op.As<MeanOperation>());
...
absl::Status OpMixin<ConcatenationOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const ConcatenationOperation& op_data, op.As<ConcatenationOperation>());
...
absl::Status OpMixin<ReshapeOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const ReshapeOperation& op_data, op.As<ReshapeOperation>());
...
absl::Status OpMixin<ResizeBilinearOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const ResizeBilinearOperation& op_data, op.As<ResizeBilinearOperation>());
  if (op.inputs.size() != 2) {
    return absl::InvalidArgumentError(
...
OpMixin<ResizeNearestNeighborOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const ResizeNearestNeighborOperation& op_data, op.As<ResizeNearestNeighborOperation>());
  if (op.inputs.size() != 2) {
    return absl::InvalidArgumentError(
        absl::StrFormat("%s expects 2 inputs (input, size)", op_name));
  }
...
absl::Status OpMixin<SpaceToDepthOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const SpaceToDepthOperation& data, op.As<SpaceToDepthOperation>());
...
absl::Status OpMixin<DepthToSpaceOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const DepthToSpaceOperation& data, op.As<DepthToSpaceOperation>());
...
absl::Status OpMixin<TransposeConvOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const TransposeConvOperation& op_data, op.As<TransposeConvOperation>());
  if (op.inputs.size() != 3) {
    return absl::InvalidArgumentError(absl::StrFormat(
...
absl::Status OpMixin<TransposeConv2DOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const TransposeConv2DOperation& op_data, op.As<TransposeConv2DOperation>());
  if (op.inputs.size() < 3) {
    return absl::InvalidArgumentError(absl::StrFormat(
...
absl::Status OpMixin<SplitOperation, XnnpackMixinTag>::ToXnnpack(
  ...
  LRT_TENSOR_ASSIGN_OR_RETURN(const SplitOperation& data, op.As<SplitOperation>());
